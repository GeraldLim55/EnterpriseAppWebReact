...
export default function LoginPage() {
  ...
  const { register: regTotp, handleSubmit: handleTotp, formState: { errors: totpErrors, isSubmitting: totpSubmitting } } = useForm({
  ...
  const onTotpSubmit = async ({ code }) => {
    try {
      const res = await authApi.verifyTwoFactor({ twoFactorToken, code })
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        const res = await authApi.googleLogin({ idToken: tokenResponse.access_token })
        if (!res.data.success) throw new Error(res.data.message)
        const { accessToken, refreshToken, session } = res.data.data
        completeLogin(accessToken, refreshToken, session)
        redirectAfterLogin()
      ...
      <div className="min-h-screen flex bg-gray-50">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          <div className="w-full max-w-sm">
            <div className="flex items-center justify-center w-12 h-12 rounded-full bg-brand-100 mb-6 mx-auto">
              <ShieldCheck className="w-6 h-6 text-brand-600" />
            </div>
            <h1 className="text-2xl font-semibold text-gray-900 mb-1 text-center">Two-factor verification</h1>
            <p className="text-sm text-gray-500 mb-8 text-center">
              Enter the 6-digit code from your authenticator app.
            </p>
            <form onSubmit={handleTotp(onTotpSubmit)} className="flex flex-col gap-4">
              <Input
                ...
                {...regTotp('code')}
              />
              <Button type="submit" loading={totpSubmitting} className="w-full">
                Verify
              </Button>
              <button
                type="button"
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs text-gray-400 bg-gray-50 px-3 w-fit mx-auto">or</div>
          </div>
